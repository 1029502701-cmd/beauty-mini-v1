...
export class BeautyReportRepository {
  ...
  async createReport(input: CreateReportInput): Promise<CreateReportResult> {
    ...
    const sql = "INSERT INTO beauty_reports (id, user_id, image_id, level, status, face_metrics_json, analysis_json, analysis_version, created_at, expire_at, image_url, thumbnail_url, decision_answers_json) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
