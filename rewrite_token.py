...
export const fetchTokenOrders = async (filter: TokenOrderFilter = {}): Promise<PaginatedResult<TokenOrder>> => {
  ...
  return callOrFallback(
    () => apiClient.get<ApiResponse<{ items: TokenOrder[]; total: number; page: number; pageSize: number; totalPages: number }>>(`${API_PATH}/orders?${params.toString()}`),
    undefined!
  ).then((res) => {
...
export const updatePackageStatus = async (id: string, status: TokenPackage["status"]): Promise<void> => {
  await callOrFallback(
    () => apiClient.patch<void>(`${API_PATH}/packages/${encodeURIComponent(id)}/status`, { status }),
    undefined
...
"""
with open(r"C:\Users\yao\Documents\Ai美妆\admin\beauty-admin\src\services\tokenService.ts", "w", encoding="utf-8") as f:
    f.write(content)
print("tokenService.ts rewritten, len:", len(content))
