# Fix reportService.ts
path = r"C:\Users\yao\Documents\Ai美妆\admin\beauty-admin\src\services\reportService.ts"
...
const MOCK_REPORTS: BeautyReport[] = [
  { id: "r001", userId: "u001", userNickname: "小美", imageUrl: "", faceShape: "鹅蛋脸", eyeShape: "桃花眼", skinTone: "暖白皮", overallScore: 88, level: "advanced", createdAt: "2026-07-30T10:00:00Z", status: "completed", unlockStatus: "free" },
  ...
  { id: "r005", userId: "u001", userNickname: "小美", imageUrl: "", faceShape: "鹅蛋脸", eyeShape: "桃花眼", skinTone: "暖白皮", overallScore: 0, level: "beginner", createdAt: "2026-07-28T16:00:00Z", status: "failed", unlockStatus: "locked" },
];
...
export const fetchReports = async (filter: ReportFilter = {}): Promise<PaginatedResult<BeautyReport>> => {
  ...
  return callOrFallback(
    () => apiClient.get<ApiResponse<{ items: BeautyReport[]; total: number; page: number; pageSize: number; totalPages: number }>>(`${API_PATH}?${params.toString()}`),
    undefined!
  ).then((res) => {
...
export const deleteReport = async (reportId: string): Promise<void> => {
  await callOrFallback(
    () => apiClient.delete<void>(`${API_PATH}/${encodeURIComponent(reportId)}`),
    undefined
