...
const Index = () => {
  ...
  const performFullAnalysis = useCallback(async () => {
    ...
    try {
      ...
      // Delete the original selfie photo after analysis is complete
      // Face metrics are preserved; only the raw photo is removed
      if (imageKey) {
        uploadService.deleteOriginalPhoto(imageKey).catch(err => {
          console.warn("[analyzing] failed to delete original photo:", err);
        });
      }

      setTimeout(() => {
        const resultReportId = generatedReportId || uploadId;
        navigate("/pages/result/index?reportId=" + encodeURIComponent(resultReportId));
      }, 800);
    } catch (error) {
