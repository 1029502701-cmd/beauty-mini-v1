...
export const fetchOperationLogs = async (filter: OperationLogFilter = {}): Promise<PaginatedResult<AdminOperationLog>> => {
  ...
  return callOrFallback(
    () => apiClient.get<ApiResponse<{ items: AdminOperationLog[]; total: number; page: number; pageSize: number; totalPages: number }>>(url),
    undefined!
  ).then((res) => {
...
export const createOperationLog = async (data: {
  adminId: string; adminName: string; actionType: string;
  targetType: string; targetId: string; targetName?: string; detail?: string;
}): Promise<string> => {
  try {
    const res = await apiClient.post<ApiResponse<{ id: string }>>(API_PATH, data);
    return res?.data?.id || '';
  } catch {
