...
export const fetchProducts = async (filter: ProductFilter = {}): Promise<PaginatedResult<Product>> => {
  ...
  return callOrFallback(
    () => apiClient.get<ApiResponse<{ items: Product[]; total: number; page: number; pageSize: number; totalPages: number }>>(url),
    undefined!
  ).then((res) => {
...
export const fetchProductDetail = async (productId: string): Promise<Product> => {
  const url = API_PATH + '/' + encodeURIComponent(productId);
  return callOrFallback(
    () => apiClient.get<ApiResponse<Product>>(url),
    undefined!
  ).then((res) => {
...
export const updateProduct = async (id: string, data: Partial<Product>): Promise<void> => {
  const url = API_PATH + '/' + encodeURIComponent(id);
  await callOrFallback(
    () => apiClient.patch<void>(url, data),
    undefined
  ).catch(() => { /* no-op fallback */ });
};

export const updateProductTags = async (id: string, recommendedTags: string[]): Promise<void> => {
  const url = API_PATH + '/' + encodeURIComponent(id) + '/tags';
  await callOrFallback(
    () => apiClient.patch<void>(url, { recommendedTags }),
    undefined
  ).catch(() => { /* no-op fallback */ });
};
